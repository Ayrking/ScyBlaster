...
public final class ResourceManager {
    ...
    public static PreparedResources prepare_resources(@NotNull ArrayList<Resource> resources) {
        ...
        int idx = 0;
        CompletableFuture<Boolean>[] futures = new CompletableFuture[HANDLERS.size()];
        ...
        if (response.number_of_error() != 0) {
            LOGGER.error(
                    String.format("%1d/%2d files could not be loaded !", response.number_of_error(), resources.size()));
        }
