...
public class Runner {
     ...
     */
    /*
     * public Runner(@NotNull Configs configs, @NotNull Version version, @Nullable
     * GameConfigs game,
     * 
     ...
     * }
     * logger.warn(String.format("Minecraft closed with code %d",
     * proc.exitValue()));
     * 
     * } catch (IOException e) {
     * 
     ...
     * for (int valIdx = 0; valIdx < version.arguments.jvm[idx].values.length;
     * valIdx++) {
     ...
     * for (int valIdx = 0; valIdx < version.arguments.game[idx].values.length;
     * valIdx++) {
     * boolean valHasValue = ((valIdx + 1) <
     * version.arguments.game[idx].values.length)
     * && !version.arguments.game[idx].values[valIdx + 1].substring(0,
     * 2).equals("-");
     * 
     * boolean toCheck = true;
     ...
     * return MCArguments.MULTIP_QUICKPLAY + " " +
     * configs.getMinecraftConfigs().getQuickPlayM() + " ";
     ...
     * }
     * 
     * private String makeJVMArguments(int argIndex, int valIdx, @NotNull ClassPath
     * cp) {
     ...
     * return version.arguments.jvm[argIndex].values[valIdx].replace(MCArguments.
     * NAVTIVES_PATH,
     * ResourceHandler.getLibsDir()) + " ";
     * 
     * case MCArguments.LAUNCHER_BRAND:
     * return MCArguments.LAUNCHER_BRAND.replace(MCArguments.PLACEHOLDER_BRAND,
     * "Scyblaster") + " ";
     * case MCArguments.LAUNCHER_VERSION:
     * return MCArguments.LAUNCHER_VERSION.replace(MCArguments.PLACEHOLDER_VERSION,
     * "1") + " ";
